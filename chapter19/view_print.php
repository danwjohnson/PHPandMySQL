...
if (isset($_GET['pid']) && filter_var($_GET['pid'], FILTER_VALIDATE_INT,
    array('min_range' => 1)) ) {    // Make sure there is a print ID!
    
    $pid = $_GET['pid'];
    
    // Get the print info:
    require ('../mysqli_connect.php');      // Connect to the database
    $q = "SELECT CONCAT_WS(' ', first_name, middle_name, last_name) AS artist,
    print_name, price, description, size, image_name FROM artists, prints WHERE
    artists.artist_id=prints.artist_id AND prints.print_id=$pid";
    ...
    if (mysqli_num_rows($r) == 1) {     // Good to go!
        ...
        echo '<p align="center">' . ((is_null($row['description'])) ? '(No description avaialabe)' :
        $row['description']) . '</p>';
